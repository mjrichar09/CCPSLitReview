...
export function loadConfig(config = rawConfig, { env = process.env } = {}) {
  ...
  const historyBack = config.history?.back;
  if (!Number.isInteger(historyBack) || historyBack < 0) {
    at('history.back', `must be an integer >= 0 (0 disables cross-month context), got ${JSON.stringify(historyBack)}`);
  }
