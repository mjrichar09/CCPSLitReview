 ...
 */
export async function write({ items, narratives, top_items, summary, references, month, config, health, run_stats, dry, force }) {
  ...
  const report = {
    ...
    summary,
    // Footnote markers in `summary` resolve against these — see
    // FootnoteText.jsx. Not validated here: an id that isn't a real item on
    // this page just renders as plain "[1]" text, the same graceful
    // degradation an unrecognised category-narrative reference gets.
