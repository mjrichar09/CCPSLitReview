...
/**
 * The genuine open-access full-text link, when Europe PMC actually hosts one
 * (`resultType: 'core'`, requested above, is what makes `fullTextUrlList`
 * present at all) — otherwise null, so the caller falls back to the DOI
 * landing page exactly as before. Never a workaround for a real paywall:
 * `availabilityCode === 'OA'` is Europe PMC's own designation, not a guess.
 * The html view is preferred over the pdf one to match how every other link
 * on this site opens a normal page rather than downloading a file.
 */
function openAccessUrl(r) {
  const links = r?.fullTextUrlList?.fullTextUrl ?? [];
  const oa = links.find((l) => l?.availabilityCode === 'OA' && l?.documentStyle === 'html');
  return oa?.url ?? links.find((l) => l?.availabilityCode === 'OA')?.url ?? null;
}
