...
export async function fetchCategory({ category, settings, window, limiters }) {
  ...
  const { records, seen, oldestSeen } = parseFeed(xml, category.id, window);

  const notes = [];
  if (seen > 0 && records.length === seen) {
    notes.push(`all ${seen} results were in-window — the window may extend past max_results`);
  }
  if (oldestSeen) notes.push(`scanned ${seen} results back to ${oldestSeen}`);
...
export function parseFeed(xml, categoryId, window) {
  ...
  return { records, seen: entries.length, oldestSeen };
}
