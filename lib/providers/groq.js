...
export function createGroqProvider({ model, apiKey = process.env.GROQ_API_KEY, maxTokens = 8192 }) {
  ...
  return {
    ...
    async complete({ system, user, schema, maxTokens: callMax = maxTokens }) {
      ...
      // budget on reasoning first, so a budget of 1024 was consumed before any
      // JSON was emitted and Groq answered 400 json_validate_failed with an
      // empty `failed_generation` — a truncation reported as a schema error.
