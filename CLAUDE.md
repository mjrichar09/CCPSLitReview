...
- **All source text goes through `cleanText`, and `cleanText` decodes every entity.** Sources emit both numeric forms — PubMed hex (`&#x2011;`, `&#x3b1;`, accented author names), RSS decimal (`&#8217;` for a curly apostrophe) — and the viewer renders anything undecoded literally, which is how `What&#8217;s` reached the site. Decoding happens in one pass so `&amp;` cannot cascade into a following `#8217;`, entity-encoded markup (`&lt;sup&gt;`) is re-stripped against a tag whitelist afterwards, and typographic punctuation is folded to ASCII while accented *letters* are preserved — author names must stay intact. `test/cleanText.test.js` guards all of this.
- **Generated prose is plain ASCII in American spellings; source text is never reworded.** Titer not titre, behavior not behaviour, modeling not modelling. The rule lives in all three prompts (`docs/digest-routine-prompt.md`, `lib/pipeline/summarize.js`, `lib/pipeline/synthesize.js`) because either generator can write a month. Titles, author names and venues are quoted material: fix their *encoding*, never their wording or spelling.
