...
**Vote-based sorting never sorted anything.** Reported as "I upvoted a paper and it didn't move." The fix that shipped for it (`.key` from `Children.toArray()`) was itself wrong for a second, deeper reason once actually tested against a running server: `ItemRow` is a Server Component, so what crosses into the client-side sorter as `children` is ItemRow's *already-rendered* output, not a `<ItemRow item={...}>` element — the `item` prop never reaches the client at all, by construction. Caught by adding a live `window.__DEBUG_SORT__` trace and watching every derived id come back `null`, not by reasoning about it in the abstract. Real fix: `CategoryPage` now passes `itemIds` as a plain, index-aligned prop, sidestepping the Server Component boundary entirely. Verified with Playwright against a real production build and a mocked `vote_tallies` response — a 9-vote paper actually jumps to the top.

**@mention notifications had never once fired.** Checked directly against the live project rather than assumed: `notifications` had 0 rows against 7 real comments, 3 of them genuine mentions. The trigger's regex (`@([A-Za-z0-9 _-]{2,32})`) captured too much (no punctuation in the class, so it swallowed the sentence after the name) or too little (a name with parentheses, "Yan Chun Yeoh (Jason)", can never be captured at all). Rewritten to check literal substring containment against each approved display name instead of extracting-then-comparing — sidesteps both failure modes because it never tries to bound the match with a character class. Verified against the three real failed mentions before applying, then backfilled the three notifications that should have fired.

**Three new features**, all requiring the same approval gate as voting/commenting since they're personal state, not public content: manual read-marking, favorites with a new `/digest/favorites` page, and a `/digest/discussion` board listing every commented-on paper across every month, pre-expanded. Comments also gained edit/delete for their own author — the schema (`edited_at`/`deleted_at`, `comments_update_own`) already supported it from Phase 6, only the UI was missing. The two new pages needed content spanning every month at once, which no existing page did — `lib/allItems.js` scans the whole archive once, and `FavoritesList`/`DiscussionList` reuse the exact "server renders everything, a client leaf decides what's shown" shape `SortableItemList` established, itemIds-as-parallel-prop included. `Engagement`'s `month` prop became `itemMonths` (item_id -> month) as part of this, since a page spanning many months has no single month to attribute a new vote or comment to.

**The site gained a real access gate.** Previously `/` redirected straight to the latest month — fully public, no login wall at all. Now `proxy.js` (Next 16 renamed Middleware to Proxy; using the current convention rather than the deprecated one) checks every `/digest/**` request at the edge against a real session and `profiles.approved`, before any content is served — not a client-side redirect, an actual gate. Deliberately fails *closed* if Supabase isn't configured, the one place in this codebase where fail-open would be a real vulnerability rather than graceful degradation. The decision logic itself (`configured && user && approved`) lives in `lib/accessGate.js`, split out from `proxy.js` for a mundane but real reason: `next/server` cannot be resolved outside Next's own build, so nothing that imports it can be unit tested with plain `node --test` — the decision now can be, everything else stays verified against a real running server. A new landing page at `/` carries the sign-in buttons and an "awaiting approval" state for anyone already signed in.

**A signup notification**, backlog item finally built: a `pg_net` trigger on `profiles` insert POSTs to ntfy.sh — no email service, no API key, just a long random topic name standing in for access control. Verified for real, not just applied: a manual `net.http_post` call was checked against `net._http_response` and came back `200`, ntfy's own echo confirming the message was actually published.

Every fix and feature in this entry was checked against something real before being called done — a live Supabase project's actual row counts, a running production build hit with curl or Playwright, or (for `pg_net`) the async HTTP response log — rather than trusted from reading the code. Two of the five features that shipped in the previous session needed exactly that kind of check to catch what code review alone had missed.

## 2026-08-17 — five reader UX improvements

Five requested improvements landed on `claude/ux-improvements-cn248b`: draggable per-reader category order, live vote-sorted item lists, votes finally reaching the score stage (Phase C, previously only designed), @mentions with a notification bell, a comment-count badge, and a manual dark/light toggle. Not yet merged or deployed — see TODO.md for what has to happen first.

**Every new client piece stayed a leaf.** The architectural constraint going in was CLAUDE.md's "no page or layout becomes dynamic" — `Engagement.jsx` already established the pattern (a client context wrapping server-rendered children) and every new interactive piece reused it rather than inventing a second approach. The one that needed care: vote-sorted lists. `SortableItemList.jsx` receives `CategoryPage`'s already-server-rendered `<ItemRow>` elements as `children` and reorders the *elements*, never importing or rendering `ItemRow` itself — so `CategoryPage` and `ItemRow` are both still plain server components, and the page still prerenders in relevance order (tallies are an empty `Map` on first paint, so there is no hydration mismatch). Reordering is a small hand-rolled FLIP animation — capture each item's `getBoundingClientRect()` before the reorder, then slide from the old position — rather than a library, matching the repo's minimal-dependency ethos.

**Votes now feed `score.js`, closing out the design TODO.md carried since Phase 6.** `lib/feedback.js` mirrors `lib/util/history.js` on purpose: same "read committed months, build a fenced block with a guard against over-reading" shape, with Supabase votes as the one new input. Disabled outright with no `SUPABASE_URL`/`SUPABASE_ANON_KEY` (new Actions secrets, not yet added — see TODO); once configured, a failed query is not caught, matching the spec that this is not a third fail-soft category alongside the adapters. Reuses `lib/util/history.js`'s "strictly earlier months" semantics for the same reason: only a previously-published month can have collected votes at all. `lib/supabase/server.js` is a second, deliberately separate Supabase client from `lib/supabase/client.js` — Node/Actions-side, plain `SUPABASE_URL`/`SUPABASE_ANON_KEY` rather than the browser's `NEXT_PUBLIC_` names, so the two credential sets can never cross.

**@mentions needed one real design decision: how a mention resolves unambiguously.** `profiles.display_name` had no uniqueness constraint. Rather than a hidden id-based token, display names are now unique (case-insensitive unique index), and `handle_new_user()` appends " 2", " 3", ... on a sign-up collision instead of failing the insert — not race-proof against two simultaneous signups with the same name, but that is not a real risk for a single-reader-base site and the unique index is the actual backstop if it ever happened. The mention trigger (`notify_comment_mentions`, on `comments` insert) follows the existing `enforce_comment_rate_limit` shape exactly: `security definer`, pinned `search_path`, revoked from `PUBLIC` in the same migration rather than left for a later hardening pass. The harder problem was the deep link: there is no runtime way to resolve an `item_id` to its category (report data is build-time-only, no API routes), so `comments` gained a `category_id` column populated client-side, where `ItemRow`/`CategoryPage` already know it — the same trick that lets "Also appears in" and the Top 5 work with no lookup. One adjacent bug surfaced by this: the `:target` CSS override that force-opens a deep-linked paper never sets the native `<details open>` attribute or fires `toggle`, so `Comments.jsx`'s lazy-load (which listens for `toggle`) would not have fired for a reader arriving from a notification link. Fixed by also checking `details.matches(':target')` on mount.

**A new-to-this-repo ESLint rule, `react-hooks/set-state-in-effect`, caught three of the new components** (`ThemeToggle`, `CategoryNav`, `Notifications`) calling `setState` synchronously in an effect body — the existing codebase never tripped it because `SessionProvider`/`Engagement`'s effects only ever set state from inside a Supabase `.then()` callback. Fixed the same way: wrap the read (`localStorage`, `matchMedia`, or the initial fetch) in `queueMicrotask` so the state update is no longer syntactically "direct."

**Verified end to end in a real browser (Playwright, Chromium) against a local production build**, not just `npm test`/`npm run build`: dark-mode toggle actually repaints the page and survives a reload via the pre-hydration script; dragging a category pill actually reorders it and the order survives a reload via localStorage; comment-count badges render on every collapsed row; zero console/page errors. Everything that talks to Postgres (votes, comments, mentions, notifications, the signed-in half of category-order persistence) could not be exercised locally — no Supabase credentials are configured in this environment — and needs verification against the real project after the migrations and secrets below are in place.

**Four new migrations, not yet applied to the live project:** `category_order` (a `jsonb` column on `profiles`, writable by any signed-in user since it is a display preference, not content — `profiles_update_own`'s existing `with check` only pins `approved`, so no new policy was needed), `comments.category_id`, the unique-display-name index + updated `handle_new_user`, and `notifications` + its trigger. The unique index is the one with real deployment risk: if two already-approved readers currently share a display name, it fails to create until one is renamed by hand.
